...
def test_cossin_rotary_kernel_matches_upstream():
    from torchtitan.models.common.rope import CosSinRoPE

    from ascend_titan.kernels.rope import npu_rotary_cossin

    dev = torch.device("npu:0")
    cfg = CosSinRoPE.Config(dim=64, max_context_length=128)
    with torch.device(dev):
        ref = cfg.build()
        new = npu_rotary_cossin(cfg).build()
    torch.manual_seed(0)
    q = torch.randn(100, 8, 64, device=dev, dtype=torch.bfloat16)
    k = torch.randn(100, 2, 64, device=dev, dtype=torch.bfloat16)
    pos = torch.cat([torch.arange(60), torch.arange(40)]).to(dev)
    q1, k1 = ref(q, k, pos)
    q2, k2 = new(q, k, pos)
    torch.testing.assert_close(q1, q2, atol=3e-2, rtol=3e-2)
    torch.testing.assert_close(k1, k2, atol=3e-2, rtol=3e-2)
