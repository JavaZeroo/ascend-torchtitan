...
- **参考 recipe 回到上游默认的 spmd 后端**：删除 DELTA 2（`spmd_backend="partial_dtensor"`）。TT-5 / TORCH-6 是"FSDP2 只在 torch >= 2.14.0.dev 上读 spmd_types 注解"的版本差，NIGHTLY 上不存在。四条 golden 重录后**与 partial_dtensor 时逐位相同**——两个后端在这条路径上数值等价。非默认后端保留为探针 `qwen3_debugmodel_npu_partial_dtensor`。参考 recipe 至此只剩两条增量（attention 节点 + 关 checkpoint I/O），由 `test_reference_recipe_keeps_only_two_deltas` 守住。
- **`npu_baseline` 拆分（P12，关闭 OURS-12）**：`npu_minimal`（矩阵默认施加，只含"不加就跑不起来"的增量）+ `npu_fused`（drop-in 融合算子，opt-in）。`npu_rms_norm` 从基线移到 `npu_fused`——性能 override 留在基线里会让矩阵红格分不清是上游问题还是我们的内核。矩阵 CLI `--stock` 改为 `--mode minimal|stock|fused`，动态 recipe 后缀 `__stock` / `__fused`。NPU 实测三种模式：minimal 4.01819、fused 4.01816（tps 81k → 94k，显存 0.72 → 0.63 GiB）、stock 如期停在 DEP-INDUCTOR。
