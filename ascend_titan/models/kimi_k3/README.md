...
| `kimi_k3_debugmodel_npu_fused` | 再叠加 ops-nn SiTU-GLU（需要装 ops-nn run 包并构建 `cann_ops_nn`） |
...
- SiTU-GLU：在 NIGHTLY venv 里构建 `cann_ops_nn`，跑 `kimi_k3_debugmodel_npu_fused`。
