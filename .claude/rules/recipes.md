...
- **族增量声明一次，任何 flavor 自动可用**：模型包在 `recipes.py` 里写一个 `npu_deltas(config)`（这个模型族在昇腾上需要什么，与尺寸无关），`__init__.py` 用 `_auto.npu_entry_points(_upstream, npu_deltas)` 接上——于是上游每个 flavor 都有两个入口——裸名（上游原样，对照组）与 `<flavor>_npu`（叠加族增量），**上游新增尺寸不需要我们写任何代码**。手写 recipe 只在需要额外东西时才写（真实资产、并行布局、golden 冻结的冒烟配置），并且必须调用 `npu_deltas`，保证族增量只存在一处。
