...
recipe 里激活：`add_override(config, <OP>_OVERRIDE)`（常量在 `ascend_titan/kernels/__init__.py`）。要让**矩阵**也自动施加它：没有它就跑不起来 ⇒ 加进 `recipes/matrix.py::npu_minimal`；只是更快（drop-in）⇒ 加进同文件的 `npu_fused`——性能项进 minimal 是 P12 违规。
