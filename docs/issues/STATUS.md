...
| TT-2 | `set_timeout` 无特性检查 | 已确认 | 本地补丁：`_set_pg_timeout` 回退 | 待做 |
...
| TT-8 | PP `step(arg_mbs=)` 无 fallback | 已确认 | 本地补丁：`_step_microbatches` 回退 | 待做 |
| TT-9 | fused_mla 用 nightly-only `torch.Tag.inplace` | 已确认 | 本地补丁：`getattr` 守卫 | 待做 |
