...
| NPU-11 | `NPUTritonKernel.add_npu_inductor_meta` 无条件写 `simd_simt_mix`，910B2 上 autotune 因此发出 `--pure-simt`，bishengir 不认识 | 未提交（与 TA-1 一起提） | `patches/torch_npu/NPU-11-simt-kernel-type-chip-gate.patch`（按 `is_ascend950` 分流） | 复现 ✅（`qwen35_debugmodel` stock，抓到 autotune 候选 `--num-warps=64/32/16/8`）；修复验证 ⚠️ SIMT 参数消失，但同一 kernel 改在 SIMD 路径失败（`LLVM ERROR: Failed to obtain op buffer shape size which should be static.`）——报错变诚实，功能未解锁 |
