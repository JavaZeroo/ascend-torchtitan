...
| NPU-11 | `NPUTritonKernel.add_npu_inductor_meta` 无条件写 `simd_simt_mix`，910B2 上 autotune 因此发出 `--pure-simt`，bishengir 不认识 | 未提交（与 TA-1 一起提） | `patches/torch_npu/pending/NPU-11-simt-kernel-type-chip-gate.patch`（按 `is_ascend950` 分流） | 复现 ✅（`qwen35_debugmodel` stock，抓到 autotune 候选 `--num-warps=64/32/16/8`）；修复验证 ⚠️ SIMT 参数消失，但同一 kernel 改在 SIMD 路径失败（`LLVM ERROR: Failed to obtain op buffer shape size which should be static.`）——报错变诚实，功能未解锁 |
| NPU-12 | `create_block_mask` 的三个新关键字（`separate_full_blocks` 等）streaming 构建器不接受，每次调用抛 TypeError 后静默退回 dense 构建 | 未提交 | `patches/torch_npu/pending/NPU-12-streaming-block-mask-kwargs.patch`（三个参数全部转发） | 复现 ✅（`qwen35_debugmodel` 每步一条 WARNING）；修复验证 ⏳ 未跑 |
...
| TA-1 | **triton-ascend 3.2.2 内部不自洽**：Python 前端在 SIMT 路径发 `--enable-triton-ir-compile` / `--pure-simt`，自带的 bishengir-compile 完全不支持 SIMT （`--help` 里 0 个相关参数） | 待提（`gitcode.com/Ascend/triton-ascend`，在授权范围内） | 正文见 `docs/issues/triton-ascend.md#ta-1`；触发者是 torch_npu inductor 对 indirect load+sum 选 `simt_only`，flex 的 document mask 命中 | 复现 ✅（`strings bishengir-compile | grep -ci simt` = 0，`--help-hidden` 同样 0）；**影响面 2026-09-02 重新界定：挡住的矩阵格子 0 个**——掩码构建改走 eager 后，原先归因 TA-1 的三个 qwen3_5 用例全部转绿（53s / 72s / 59s）。是正确性缺陷（声明支持却不可执行 + 误导性报错），不是阻塞 |
