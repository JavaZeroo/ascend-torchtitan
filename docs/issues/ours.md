...
| OURS-10 | gpt_oss + **TP** 失败（已 bisect）：tp1/ep4 🟢（loss 3.82）、pp+ep+sacop 🟢；tp2/ep4 → `softmax_backward` 形状 `[512, 8]` vs `[256, 8]`；tp2/ep1 → `HcclReduceScatter` 内部错误在 `CosSinRoPE._reshape_cache` 的同步点暴露。llama3/qwen3 的 TP2 用例均 🟢，所以是 gpt_oss 在 TP 下特有：候选是 sinks 参数的 DTensor `to_local(grad_placements=)` 与我们返回的 LSE（非 DTensor、(T,N)）的交互，或 gpt_oss 的 SP 分片下 T 不一致。 | 已二分：去掉 RMSNorm override 仍失败（attention + rope 必需，无法再去）；关 SP 后变成 `Can not redistribute from S(0) to P(sum)`（DTensor 重分布，gpt_oss + TP 无 SP 路径）。三种表现都只在 gpt_oss × TP 出现；llama3/qwen3 TP2 🟢。下一步在 GPU 上对照跑 stock 同配置以判断是否上游本身问题。 |
