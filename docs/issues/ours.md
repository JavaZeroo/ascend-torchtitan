...
| OURS-8 | ~~`torch.compile` 下 dynamo 追踪进 `npu_fusion_attention`……~~ **已解决（M3）**：前向/反向封装为 `custom_op` + `register_fake`，`cu_seq` 张量入参、D2H 在算子内；`fullgraph=True` + `aot_eager` 通过。`*_compile` 用例 2026-09-01 装上 Triton-Ascend 后重跑：inductor 能编，它们改为死在 flex 的 document mask 上（硬件门，Ascend950）。 | 关闭 |
| OURS-9 | ~~`npu_baseline` 加的 override 与上游 `fused_mla` 抢节点~~ **已关闭（2026-08-30）**：`fused_mla` 认领的是 `layers.N.attention`，它同时是 inner attention 节点和 RoPE 节点的祖先，所以**两个** override 都不能加（此前只跳过了 RoPE）。修复后该用例归因变为 `TT-KERNEL`（上游树内的 CUDA-only Triton 内核）。 | 关闭 |
