...
| OURS-8 | `torch.compile` 下 dynamo 追踪进 `npu_fusion_attention`，host offsets 变成 unbacked SymInt 后 fake 调用失败（矩阵 `1d_compile` 等）。 | 已把 `AscendFusionAttention.forward` 标为 `torch.compiler.disable`（内核对 dynamo 不透明，其余仍可编译）；后续改用 `custom_op` + `register_fake` 让它可追踪。 |
