...
| **M5** 🔄 | 图模式 / 低精度 / 多模态 | `graph/`（torchair）、post-converter 树上的 FP8 override、多模态轴 | 矩阵轴扩展；不新增机制类型。**进展：** 图模式 `components=["loss"]` 🟢（torchair，需 `TORCHAIR=1` 重建 torch_npu）、`["model"]` 🔴 OURS-13（自定义算子无 GE converter）；多模态轴由 kimi_k3 覆盖 🟢；FP8：`zero_` 已修（op-plugin NPU-6 扩展到 float8），张量可用；`_scaled_mm` 要 Ascend950 及以后，910B2 没有 FP8 计算单元，因此不写 FP8 recipe。
