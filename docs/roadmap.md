...
| **M2** | 能力矩阵 | 上游 `tests/integration_tests` 用例在 NPU 上扫描（`tools/matrix.py`）、矩阵三态记录、nightly CI 两条腿（pinned + 最远 SHA）、归因自动初判 | M2 各轴（并行 × 注意力 × AC × compile）无 ⚪；每个 🔴 有归因。 |
| **M3** | override 机制加固 | provenance 表、`opcheck`、第二个 override（SituGLU 或 RMSNorm）、注意力的 LSE 尾部（CP） | 每次运行日志带 provenance；两个 override 带对齐测试。 |
